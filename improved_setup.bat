...
powershell -Command "Invoke-WebRequest -Uri 'http://files.portaudio.com/archives/pa_stable_v190700_20210406.tgz' -OutFile 'portaudio.tgz'" || goto portaudio_manual_setup
...
echo Creating minimal PortAudio implementation...

:: Write a dummy portaudio_x64.lib file
echo Creating dummy PortAudio library...
echo LIBRARY PORTAUDIO > portaudio_x64.def
...
copy ..\libs\portaudio\include\portaudio.h .
echo // Mock PortAudio implementations > portaudio_impl.c
echo #include "portaudio.h" >> portaudio_impl.c
echo #ifdef __cplusplus >> portaudio_impl.c
echo extern "C" { >> portaudio_impl.c
echo #endif >> portaudio_impl.c
echo PaError Pa_Initialize(void) { return paNoError; } >> portaudio_impl.c
echo PaError Pa_Terminate(void) { return paNoError; } >> portaudio_impl.c
echo int Pa_GetDeviceCount(void) { return 1; } >> portaudio_impl.c
echo const PaDeviceInfo* Pa_GetDeviceInfo(PaDeviceIndex device) { static PaDeviceInfo info = {0}; info.name = "Mock Device"; return &info; } >> portaudio_impl.c
echo PaError Pa_OpenStream(PaStream** stream, const PaStreamParameters* inputParameters, const PaStreamParameters* outputParameters, double sampleRate, unsigned long framesPerBuffer, PaStreamFlags streamFlags, PaStreamCallback* streamCallback, void* userData) { static int dummy = 0; *stream = &dummy; return paNoError; } >> portaudio_impl.c
echo PaError Pa_CloseStream(PaStream* stream) { return paNoError; } >> portaudio_impl.c
echo PaError Pa_StartStream(PaStream* stream) { return paNoError; } >> portaudio_impl.c
echo PaError Pa_StopStream(PaStream* stream) { return paNoError; } >> portaudio_impl.c
echo PaError Pa_IsStreamActive(PaStream* stream) { return 0; } >> portaudio_impl.c
echo PaTime Pa_GetStreamTime(PaStream* stream) { return 0; } >> portaudio_impl.c
echo const char* Pa_GetErrorText(PaError errorCode) { return "Mock error"; } >> portaudio_impl.c
echo PaDeviceIndex Pa_GetDefaultInputDevice(void) { return 0; } >> portaudio_impl.c
echo PaDeviceIndex Pa_GetDefaultOutputDevice(void) { return 0; } >> portaudio_impl.c
echo PaError Pa_IsFormatSupported(const PaStreamParameters* inputParameters, const PaStreamParameters* outputParameters, double sampleRate) { return paFormatIsSupported; } >> portaudio_impl.c
echo #ifdef __cplusplus >> portaudio_impl.c
echo } >> portaudio_impl.c
echo #endif >> portaudio_impl.c

copy portaudio_impl.c ..\libs\portaudio\lib\
copy portaudio_x64.def ..\libs\portaudio\lib\
...
echo LIB.EXE /OUT:..\libs\portaudio\lib\portaudio_x64.lib > ..\libs\portaudio\lib\dummy.txt
